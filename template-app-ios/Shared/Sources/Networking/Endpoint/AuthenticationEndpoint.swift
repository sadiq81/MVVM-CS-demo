...
extension AuthenticationEndpoint: Endpoint {
    ...
    var path: String {
        switch self {
            case .register:
                // Registration is handled by the backend's UserController (POST /api/user), not the auth group.
                return "/user"
            case .login:
                return "/auth/login"
            case .accessToken:
...
extension AsyncNetworkService: @retroactive RefreshTokenServiceType {
    ...
    nonisolated public func endpoint(refreshToken: String) throws -> Endpoint {
        let service: any InstallationServiceType = Container.shared.installationService()
