...
protocol OnboardingServiceType: Sendable {
...
final class OnboardingService: OnboardingServiceType, @unchecked Sendable {
    ...
    func onboardedCompleted(for type: OnboardingType) -> OnboardingState {
        ...
        let state: OnboardingState = (type.steps.set == completed) ? .completed : .unknown
        return state
    }
    ...
    func refresh() async throws {
        ...
        await MainActor.run { self.onboardingStates = onboardingStates }
    }
