...
final class MoreCoordinator: NSObject, UIKitCoordinatorType {
    ...
    private var secureStorage: any SecureStorageServiceType
    ...
    private func configure() {
        self.navigationController?.tabBarItem = UITabBarItem(title: Strings.Tabbar.more, image: UIImage(systemName: Images.System.more)?.withRenderingMode(.alwaysTemplate), tag: TabBarTransition.more.rawValue)
    }
        ...
        if let transition = transition as? MoreTransition {
            switch transition {
                ...
                case .webview(let url):
                    ...
                    if #available(iOS 26, *) {
                        // Do not set preferredBarTintColor on iOS 26+ (deprecated)
                    } else {
...
extension MoreCoordinator: PinDelegate {
    ...
    func didAuthenticate(data: Data) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            var viewModel = Container.shared.secretViewModelType()
            viewModel.data = data
