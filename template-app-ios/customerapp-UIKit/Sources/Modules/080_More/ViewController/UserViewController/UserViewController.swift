...
final class UserViewController: UIViewController {
    ...
    @IBOutlet weak var validateContainerView: UIView!
    ...
    private func configureView() {
        ...
        [self.profileContainerView, self.addressContainerView, self.validateContainerView].forEach {
            $0?.backgroundColor = Colors.Background.neutralSubtle.color
            $0?.layer.cornerRadius = Constants.Rounding.small
        }
        ...
        // Navigable-row affordances: street address and country open pickers, so
        // show a trailing chevron to match the SwiftUI/Mixed "Land ›" / "Vejnavn ›" rows.
        self.streetAddressTextField.rightView = self.makeChevronView()
        self.streetAddressTextField.rightViewMode = .always
        self.countryTextField.rightView = self.makeChevronView()
        self.countryTextField.rightViewMode = .always

        // Change-password row: align wording ("Skift adgangskode") and add a chevron
        // affordance so it reads as a tappable row like SwiftUI/Mixed.
        self.passwordButton.setTitle(Strings.Profile.Button.changePassword, for: .normal)
        self.passwordButton.setImage(UIImage(systemName: Images.System.forward), for: .normal)
        self.passwordButton.tintColor = Colors.Foreground.muted.color
        self.passwordButton.semanticContentAttribute = .forceRightToLeft
        self.passwordButton.contentHorizontalAlignment = .left
    ...
    /// A trailing chevron used to indicate a field navigates to a picker.
    private func makeChevronView() -> UIView {
        let imageView = UIImageView(image: UIImage(systemName: Images.System.forward))
        imageView.tintColor = Colors.Foreground.muted.color
        imageView.contentMode = .scaleAspectFit
        imageView.frame = CGRect(x: 0, y: 0, width: 12, height: 16)
    ...
    private func update(with user: UserModel) {
        ...
        if let country = user.country,
           let country = Country.getCountry(from: country) {
            ...
            self.countryTextField.country = Country.getCountry(from: user.phoneCountry ?? "")
        }
        ...
        let validateStyle: UIButton.Style = user.isBirthDateValidated ? .danger : .primary
        self.validateBirthdayButton.configure(style: validateStyle, text: validateText)
        self.validateBirthdayLabel.isHidden = true
    }
    
    private func validate(user: UserModel) -> Int {
        ...
        let phoneCountry = self.phoneCountryButton.country?.isoCountryCode ?? ""
        ...
        let country = self.countryTextField.country?.isoCountryCode ?? ""
...
extension UserViewController: AddressSearchDelegate {
    ...
    func didSelect(_ suggestion: AddressSuggestionModel) {
        ...
        self.cityTextField.text = suggestion.addressModel?.city
        self.zipCodeTextField.text = suggestion.addressModel?.postalCode
    }
}
...
#if DEBUG
#Preview("UserViewController", traits: .fixedLayout(width: 402, height: 1073)) {
    Container.shared.userViewModelType.register { MainActor.assumeIsolated { PreviewUserViewModel() } }

    let viewController = AppStoryboard.viewController(class: UserViewController.self)
