...
final class PinValidationViewController: BackgroundDimmingViewController {
    ...
    @Injected(\.pinViewModelType)
    private var viewModel: any PinViewModelType
    ...
    private func configure() {
        ...
        self.panelView.layer.cornerRadius = Constants.Rounding.medium
        self.panelView.layer.maskedCorners = [.layerMinXMinYCorner, .layerMaxXMinYCorner]
...
#if DEBUG
#Preview("PinValidationViewController") {
    Container.shared.pinViewModelType.register { PreviewPinViewModel() }

    let viewController = AppStoryboard.viewController(class: PinValidationViewController.self)
