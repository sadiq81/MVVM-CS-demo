...
final class ProductSearchViewController: UIViewController {
    ...
    @Injected(\.productSearchViewModelType)
    private var viewModel: any ProductSearchViewModelType
    ...
    private func configure() {
        // Set navigation title to match SwiftUI
        self.title = Strings.Tabbar.search

        // Set background colors to match SwiftUI
        ...
        // Match search icon container to SwiftUI (neutralSubtle background, muted tint)
        if let filterRounding = self.searchTextField.superview {
            filterRounding.tintColor = Colors.Foreground.muted.color
            filterRounding.backgroundColor = Colors.Background.neutralSubtle.color
            filterRounding.layer.cornerRadius = 8
        }
    }
    ...
    private func configureFilterCollectionViewLayout() {
        ...
        section.contentInsets = NSDirectionalEdgeInsets(top: Spacing.small, leading: Spacing.small, bottom: 0, trailing: Spacing.large)
...
#if DEBUG
#Preview("ProductSearchViewController", traits: .fixedLayout(width: 402, height: 874)) {
    Container.shared.productSearchViewModelType.register { MainActor.assumeIsolated { PreviewProductSearchViewModel() } }

    let viewController = AppStoryboard.viewController(class: ProductSearchViewController.self)
