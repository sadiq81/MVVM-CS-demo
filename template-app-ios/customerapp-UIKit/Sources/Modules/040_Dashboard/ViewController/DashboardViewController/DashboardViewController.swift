...
final class DashboardViewController: UIViewController {
    ...
    @LazyInjected(\.dashboardViewModelType)
    private var viewModel: any DashboardViewModelType
        ...
        let layout = UICollectionViewCompositionalLayout { [weak self] (sectionIndex: Int, _: NSCollectionLayoutEnvironment) -> NSCollectionLayoutSection? in
            ...
            switch content {
                ...
                case .user: // One item fill all of the screen
                    ...
                    section.contentInsets = NSDirectionalEdgeInsets(top: Spacing.medium, leading: Spacing.large, bottom: Spacing.section, trailing: Spacing.large)
                ...
                case .comment: // Two items fill all of the screen
                    ...
                    let item = NSCollectionLayoutItem(layoutSize: itemSize)
                    item.contentInsets = NSDirectionalEdgeInsets(top: 0,leading: Spacing.large,bottom: 0,trailing: Spacing.small / 2)

                    let itemSize2 = NSCollectionLayoutSize(widthDimension: .fractionalWidth(0.5), heightDimension: .fractionalHeight(1.0))
                    let item2 = NSCollectionLayoutItem(layoutSize: itemSize2)
                    item2.contentInsets = NSDirectionalEdgeInsets(top: 0,leading: Spacing.small / 2,bottom: 0,trailing: Spacing.large)

                    let groupSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1), heightDimension: .fractionalWidth(0.5))
                    let group = NSCollectionLayoutGroup.horizontal(layoutSize: groupSize, subitems: [item, item2])

                    let section = NSCollectionLayoutSection(group: group)
                    section.contentInsets = .init(top: Spacing.medium, leading: 0, bottom: Spacing.section, trailing: 0)
                ...
                case .post: // 1 item + peek of the next item
                    ...
                    let item = NSCollectionLayoutItem(layoutSize: itemSize)
                    item.contentInsets = NSDirectionalEdgeInsets( top: 0, leading: Spacing.large, bottom: 0, trailing: Spacing.small)

                    let groupSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(0.95), heightDimension: .fractionalWidth(0.4))
                    let group = NSCollectionLayoutGroup.vertical(layoutSize: groupSize, subitems: [item])

                    let section = NSCollectionLayoutSection(group: group)
                    section.orthogonalScrollingBehavior = .continuousGroupLeadingBoundary
                    section.contentInsets = NSDirectionalEdgeInsets(top: Spacing.medium, leading: 0, bottom: Spacing.section, trailing: 0)
        ...
        self.dataSource = .init(collectionView: self.collectionView) { (collectionView, indexPath, content) -> UICollectionViewCell? in
            ...
            switch content {
                ...
                case .user(let user):
                    ...
                    cell.label.attributedText = Self.userCardAttributedText(for: user)
                    cell.applyCardStyle(backgroundColor: Colors.Background.dark.color, textColor: Colors.Foreground.light.color, showBorder: false)
                    return cell
                ...
                case .post(let post):
                    ...
                    cell.applyCardStyle(backgroundColor: Colors.Background.brand.color, textColor: Colors.Foreground.light.color, showBorder: false)
                    return cell
                ...
                case .todo(let todo):
                    ...
                    cell.applyCardStyle(backgroundColor: Colors.Background.neutralSubtle.color, textColor: Colors.Foreground.default.color, showBorder: true)
                    return cell
            }
        }
    ...
    private static func userCardAttributedText(for user: UserModel) -> NSAttributedString {
        let result = NSMutableAttributedString()

        if let firstName = user.firstName {
            let attributes: [NSAttributedString.Key: Any] = [
                .font: UIFont.preferredFont(forTextStyle: .body),
                .foregroundColor: Colors.Foreground.light.color
            ]
            result.append(NSAttributedString(string: firstName, attributes: attributes))
        }

        if let email = user.email {
            let attributes: [NSAttributedString.Key: Any] = [
                .font: UIFont.preferredFont(forTextStyle: .subheadline),
                .foregroundColor: Colors.Foreground.lightMuted.color
            ]
            if result.length > 0 { result.append(NSAttributedString(string: "\n")) }
            result.append(NSAttributedString(string: email, attributes: attributes))
        }
        ...
        if let city = user.city, let country = user.country {
            ...
            if result.length > 0 { result.append(NSAttributedString(string: "\n")) }
            result.append(NSAttributedString(string: "\(city), \(country)", attributes: attributes))
        }
...
#if DEBUG
#Preview("DashboardViewController") {
    Container.shared.dashboardViewModelType.register { MainActor.assumeIsolated { PreviewDashboardViewModel() } }

    let viewController = AppStoryboard.viewController(class: DashboardViewController.self)
