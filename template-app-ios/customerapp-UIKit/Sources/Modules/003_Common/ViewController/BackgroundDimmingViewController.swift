...
#if DEBUG
#Preview("BackgroundDimmingViewController") {
    // BackgroundDimmingViewController is a code-only base class (no storyboard).
    // It adds a dimming overlay behind presented content.
    let vc = BackgroundDimmingViewController()
    return vc
}
#endif
