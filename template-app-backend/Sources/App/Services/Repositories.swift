...
extension Application {
    struct Repositories {
        struct Provider {
            static var database: Self {
                .init {
                    ...
                    $0.repositories.use { DatabaseOnboardingStateRepository(database: $0.db) }
                    $0.repositories.use { DatabaseInstallationRepository(database: $0.db) }
                }
            }
        ...
        final class Storage {
            ...
            var makeOnboardingStatesRepository: ((Application) -> OnboardingStateRepository)?
            var makeInstallationRepository: ((Application) -> InstallationRepository)?
            init() { }
        }
