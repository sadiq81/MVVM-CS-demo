...
struct OnboardingStateController: RouteCollection {
    ...
    private func list(_ req: Request) async throws -> [OnboardingStateResponse] {
        ...
        var states = try await req.onboardingStates.find(for: payload.userID)

        if states.isEmpty {
            let defaults = OnboardingState.Step.allCases.map {
                OnboardingState(userID: payload.userID, step: $0, state: .pending)
            }
            try await req.onboardingStates.create(defaults)
            states = defaults
        }

        return states.map { OnboardingStateResponse(from: $0) }
    }
