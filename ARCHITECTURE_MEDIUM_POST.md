...
final class AppCoordinator: NSObject, CoordinatorType {
    ...
    func start() {
        ...
        if self.userService.user.isNil {
            ...
            try? self.transition(to: AppTransition.main)
        }
    }
...
```swift
final class DashboardService: DashboardServiceType {
...
The project uses a **shared code architecture** where common code lives in `customerapp-Shared/` and is compiled into all three apps. App-specific code stays in each app's own directory:

```
template-app-ios/
├── customerapp-Shared/               # Shared across all 3 apps
│   ├── Common/                       # Models, Services, Networking, ViewModels
│   │   ├── Models/                   #   Domain models (Dashboard, Product, User, etc.)
│   │   ├── Services/                 #   Service protocols + implementations
│   │   ├── Networking/               #   Endpoints, Requests, Responses
│   │   ├── Extensions/               #   Foundation extensions
│   │   └── Modules/                  #   Shared ViewModelType protocols
│   └── SwiftUI/                      # Shared SwiftUI views (Mixed + SwiftUI apps)
│       ├── DesignSystem/             #   Font extensions, view modifiers
│       └── Modules/                  #   SwiftUI views + ViewStates per module
├── customerapp-UIKit/                # App 1: Pure UIKit
│   └── Sources/
│       ├── Modules/                  #   UIKit ViewControllers, Coordinators, Cells
│       ├── Extensions/               #   UIKit-specific extensions, DesignSystem
│       └── ...
├── customerapp-Mixed/                # App 2: UIKit navigation + SwiftUI views
│   └── Sources/
│       ├── Modules/                  #   Coordinators (push UIHostingController)
│       └── ...
├── customerapp-SwiftUI/              # App 3: Pure SwiftUI
│   └── Sources/
│       ├── Modules/                  #   SwiftUI Coordinators, CoordinatorViews
│       └── ...
└── customerapp.xcodeproj             # 6 targets (2 per app: Dev + Prod)
```

Each app's `Modules/` directory follows numbered module conventions:

```
Modules/
├── 001_Application/          # App delegate/entry, root coordinator
...
This architecture has proven effective in production apps with teams of 2–3 developers. It strikes a balance between structure and pragmatism, providing clear guidelines of where the responsibilites should lie.
...
- MustacheKit (Open source In house Frameworks)
